...
const DEFAULT_SYSTEM_INSTRUCTION = [
  ...
  "for coding and machine tasks, use bash first for local command execution and filesystem operations.",
  "you can also run terminal/shell commands via javascript tools (run_js/run_js_module), e.g. using child_process from javascript.",
  "for coding tasks, create/edit/save files directly via tools instead of only describing code.",
  "use javascript tools (run_js/run_js_module/install_js_packages) when js runtime/module workflows are the best fit.",
  ...
  "you can create capabilities on demand via bash/javascript tools.",
  "default execution path: run local commands with bash; for javascript-specific automation use install_js_packages then run_js or run_js_module.",
  ...
  "for actionable tasks, execute them yourself with tools instead of handing the user shell/powershell scripts.",
